...
async function getPost(pageValue) {
    const projectEl = document.querySelector('.myWork');
    ...
    if(result[0].mainImage.asset._ref) {
        projectEl.prepend(handleImage(result[0].mainImage.asset._ref));
    }
    ...
    projectEl.append(handleParagraphs(result[0].intro));

    const description = document.querySelector('.project-description');
    
    projectEl.append(handleParagraphs(result[0].description));
    
    projectEl.append(handleParagraphs(result[0].categories));
}
...
async function getPosts(highlight) {
    ...
    const worksList = document.querySelector('.myWork-home');
    ...
    result.forEach(post => {
        ...
        workBlock.classList.add('card'); // vi legger til en class til <a class="work"></a>
